...
class Register_Tanya extends \Aplikasi\Kitab\Tanya
{
#==========================================================================================
...
#---------------------------------------------------------------------------------------------------#
	public function semakPost($senarai, $post)
	{
...
#---------------------------------------------------------------------------------------------------#
	...
	{	//$this->semakPembolehubah($posmen);
		# set pemboleubah
		$password1 = $posmen[$myTable][0]['katalaluan1'];
		$password2 = $posmen[$myTable][0]['katalaluan2'];
		$password = ($password1 == $password2) ? $password1: NULL;
		$password = \Aplikasi\Kitab\RahsiaHash::rahsia('md5', $password);
		//$password = \Aplikasi\Kitab\RahsiaHash::cincang($password,20);
		$level = 'user';
		$cantum = $senaraiData = array();
		foreach ($posmen as $key => $value1):
			foreach ($value1[0] as $key2 => $dataS): //echo $key2 . '<br>';
				$cantum[] = ($key2 == 'katalaluan1') ? ($password)// . "', '"
					:(	($key2 == 'katalaluan2') ? ($level)// . "', '"
					: ($dataS)// . "', '"
					);
			endforeach;
		endforeach;
		...
		$this->semakPembolehubah($cantum);
		return $cantum; # pulangkan pemboleubah
	}
#---------------------------------------------------------------------------------------------------#
#==========================================================================================
}